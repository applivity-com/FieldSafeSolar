...
import com.applivity.fieldsafesolar.data.model.EvidenceAnalysisResult.Confidence
import com.applivity.fieldsafesolar.data.model.EvidenceAnalysisResult.Decision
import com.applivity.fieldsafesolar.data.model.InspectionType
import com.applivity.fieldsafesolar.data.model.QuestionAnswer
...
class DemoStubAnalyzer : AiSafetyAnalyzer {
    ...
    override suspend fun generateReport(request: ReportGenerationRequest): SafetyReport {
        ...
        val passedItems = request.completedChecklist.filter {
            it.status == com.applivity.fieldsafesolar.data.model.ChecklistItem.Status.COMPLETED_PASS
        }
        val warnedItems = request.completedChecklist.filter {
            it.status == com.applivity.fieldsafesolar.data.model.ChecklistItem.Status.COMPLETED_WARN
        }
        val failedItems = request.completedChecklist.filter {
            it.status == com.applivity.fieldsafesolar.data.model.ChecklistItem.Status.COMPLETED_FAIL ||
                it.status == com.applivity.fieldsafesolar.data.model.ChecklistItem.Status.COMPLETED_STOP_WORK
        }
        val pendingItems = request.completedChecklist.filter {
            it.status == com.applivity.fieldsafesolar.data.model.ChecklistItem.Status.PENDING
        }
